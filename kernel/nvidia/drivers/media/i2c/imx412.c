/*
 * imx412.c - imx412 sensor driver (based on imx477 driver)
 *
...
static inline void imx412_get_frame_length_regs(imx412_reg * regs,
						u32 frame_length)
{
	regs->addr = IMX412_FRAME_LENGTH_ADDR_MSB;
	regs->val = (frame_length >> 8) & 0xff;
	(regs + 1)->addr = IMX412_FRAME_LENGTH_ADDR_LSB;
	(regs + 1)->val = (frame_length) & 0xff;
}

static inline void imx412_get_coarse_integ_time_regs(imx412_reg * regs,
						     u32 coarse_time)
{
	regs->addr = IMX412_COARSE_INTEG_TIME_ADDR_MSB;
	regs->val = (coarse_time >> 8) & 0xff;
	(regs + 1)->addr = IMX412_COARSE_INTEG_TIME_ADDR_LSB;
	(regs + 1)->val = (coarse_time) & 0xff;
}

static inline void imx412_get_gain_reg(imx412_reg * reg, u16 gain)
{
	reg->addr = IMX412_ANALOG_GAIN_ADDR_MSB;
	reg->val = (gain >> IMX412_SHIFT_8_BITS) & IMX412_MASK_LSB_2_BITS;

	(reg + 1)->addr = IMX412_ANALOG_GAIN_ADDR_LSB;
	(reg + 1)->val = (gain) & IMX412_MASK_LSB_8_BITS;
}

static inline int imx412_read_reg(struct camera_common_data *s_data,
				  u16 addr, u8 * val)
{
	int err = 0;
	u32 reg_val = 0;
...
static inline int imx412_write_reg(struct camera_common_data *s_data,
				   u16 addr, u8 val)
{
	int err = 0;
...
static int imx412_set_group_hold(struct tegracam_device *tc_dev, bool val)
{
	...
	err = imx412_write_reg(s_data, IMX412_GROUP_HOLD_ADDR, val);
	if (err) {
...
static int imx412_get_fine_integ_time(struct imx412 *priv, u16 * fine_time)
{
	struct camera_common_data *s_data = priv->s_data;
	int err = 0;
	u8 reg_val[2];

	err = imx412_read_reg(s_data, IMX412_FINE_INTEG_TIME_ADDR_MSB,
			      &reg_val[0]);
	if (err)
		goto done;

	err = imx412_read_reg(s_data, IMX412_FINE_INTEG_TIME_ADDR_LSB,
			      &reg_val[1]);
	if (err)
		goto done;
...
static int imx412_set_gain(struct tegracam_device *tc_dev, s64 val)
{
	...
	/* Gain Formula:
	   Gain = (IMX412_GAIN_C0 - (IMX412_GAIN_C0 * gain_factor / val))
	 */
	gain =
	    (s16) (IMX412_ANALOG_GAIN_C0 -
		   (mode->control_properties.gain_factor *
		    IMX412_ANALOG_GAIN_C0 / val));

	if (gain < IMX412_MIN_GAIN)
		gain = IMX412_MAX_GAIN;
	else if (gain > IMX412_MAX_GAIN)
		gain = IMX412_MAX_GAIN;

	dev_dbg(dev, "%s: val: %lld (/%d) [times], gain: %u\n",
		__func__, val, mode->control_properties.gain_factor, gain);

	imx412_get_gain_reg(gain_reg, (u16) gain);

	for (i = 0; i < ARRAY_SIZE(gain_reg); i++) {
		err = imx412_write_reg(s_data, gain_reg[i].addr,
				       gain_reg[i].val);
		if (err) {
...
static int imx412_set_frame_rate(struct tegracam_device *tc_dev, s64 val)
{
	...
	if (frame_length < IMX412_MIN_FRAME_LENGTH)
		frame_length = IMX412_MIN_FRAME_LENGTH;
	else if (frame_length > IMX412_MAX_FRAME_LENGTH)
		frame_length = IMX412_MAX_FRAME_LENGTH;

	dev_dbg(dev,
		"%s: val: %llde-6 [fps], frame_length: %u [lines]\n",
		__func__, val, frame_length);

	imx412_get_frame_length_regs(fl_regs, frame_length);
	for (i = 0; i < 2; i++) {
		err = imx412_write_reg(s_data, fl_regs[i].addr, fl_regs[i].val);
		if (err) {
			dev_err(dev,
...
static int imx412_set_exposure(struct tegracam_device *tc_dev, s64 val)
{
	...
	int err = 0;
	imx412_reg ct_regs[2];
	const s32 max_coarse_time = priv->frame_length - IMX412_MAX_COARSE_DIFF;
	...
	if (coarse_time < IMX412_MIN_COARSE_EXPOSURE)
		coarse_time = IMX412_MIN_COARSE_EXPOSURE;
	...
	imx412_get_coarse_integ_time_regs(ct_regs, coarse_time);

	for (i = 0; i < 2; i++) {
		err = imx412_write_reg(s_data, ct_regs[i].addr, ct_regs[i].val);
		if (err) {
			dev_dbg(dev,
...
static struct tegracam_ctrl_ops imx412_ctrl_ops = {
	...
	.set_frame_rate = imx412_set_frame_rate,
...
static int imx412_set_mode(struct tegracam_device *tc_dev)
{
	...
	err = imx412_write_table(priv, mode_table[IMX412_GLOBAL_SETTING]);
	if (err)
		return err;

	err = imx412_write_table(priv, mode_table[IMX412_IMAGE_QUALITY]);
	if (err)
		return err;	

	err = imx412_write_table(priv, mode_table[s_data->mode]);
	if (err)
		return err;
...
static int imx412_start_streaming(struct tegracam_device *tc_dev)
{
	struct imx412 *priv = (struct imx412 *)tegracam_get_privdata(tc_dev);    
	dev_dbg(tc_dev->dev, "%s:\n", __func__);
	return imx412_write_table(priv, mode_table[IMX412_START_STREAM]);
}
...
static int imx412_stop_streaming(struct tegracam_device *tc_dev)
{
	...
	dev_dbg(tc_dev->dev, "%s:\n", __func__);
	err = imx412_write_table(priv, mode_table[IMX412_STOP_STREAM]);
...
static int imx412_board_setup(struct imx412 *priv)
{
	...
	err = imx412_read_reg(s_data, IMX412_MODEL_ID_ADDR_MSB, &reg_val[0]);
	if (err) {
		dev_err(dev, "%s: error during i2c read probe (%d)\n",
			__func__, err);
		goto err_reg_probe;
	}
	err = imx412_read_reg(s_data, IMX412_MODEL_ID_ADDR_LSB, &reg_val[1]);
	if (err) {
		dev_err(dev, "%s: error during i2c read probe (%d)\n",
			__func__, err);
		goto err_reg_probe;
	...
	err = imx412_get_fine_integ_time(priv, &priv->fine_integ_time);
	if (err)
			...
			const struct i2c_device_id *id)
{
	...
	priv = devm_kzalloc(dev, sizeof(struct imx412), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	dev_dbg(dev, "detected imx412 sensor\n");
...
MODULE_DESCRIPTION("Media Controller driver for Sony IMX412 (based on imx477 driver)");
