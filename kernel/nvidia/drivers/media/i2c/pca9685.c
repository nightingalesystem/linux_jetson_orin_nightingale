 ...
 *  @return returns 0 if successful
 */
static int pca9685_config(struct i2c_client *client, int duty_ns, int period_ns){
	...
	calibration_init(MAX_PWM_MODE - 2);
...
int pca9685_init(struct i2c_client *client){
	...
	ret = pca9685_config(client,duty_ns,period_ns);
	if(ret){
