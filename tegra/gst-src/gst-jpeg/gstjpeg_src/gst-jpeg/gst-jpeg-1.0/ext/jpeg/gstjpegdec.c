...
#if defined (USE_TARGET_TEGRA) || defined (USE_TARGET_GPU)
...
GST_STATIC_PAD_TEMPLATE ("src",
    ...
    (GST_CAPS_FEATURE_MEMORY_RMSURFACE,
        "{ " "I420, GRAY8, NV12, YUY2, Y42B, Y444, RGB, RGBA }") ";"
    /* TODO: Use transform functions to convert from raw->NVMM formats
     * Need to support: RGB, BGR, RGBx, xRGB, BGRx, xBGR, GRAY8
     */
    GST_VIDEO_CAPS_MAKE
        ("{ I420 }"))
    );
...
GST_STATIC_PAD_TEMPLATE ("src",
    ...
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (GST_VIDEO_CAPS_MAKE_WITH_FEATURES
    (GST_CAPS_FEATURE_MEMORY_RMSURFACE, "{ " "RGBA }") )
    );
#endif
/* *INDENT-ON* */
...
#if defined (USE_TARGET_TEGRA)
static gboolean gst_jpeg_dec_open (GstVideoDecoder * bdec);
#endif
...
G_DEFINE_TYPE (GstNvJpegDec, gst_jpeg_dec, GST_TYPE_VIDEO_DECODER);
...
gst_jpeg_dec_class_init (GstJpegDecClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_RGBA_OUTPUT,
          g_param_spec_boolean ("rgbaoutput",
              "Enable rgba output",
              "Enable rgba output",
              FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
              GST_PARAM_MUTABLE_READY));

  g_object_class_install_property (gobject_class, PROP_NV12_OUTPUT,
          g_param_spec_boolean ("nv12output",
              "Enable nv12 output",
...
#if defined (USE_TARGET_TEGRA)
  vdec_class->open = gst_jpeg_dec_open;
#endif
...
gst_jpeg_dec_fill_input_buffer (j_decompress_ptr cinfo)
{
  ...
  cinfo->src->bytes_in_buffer = cinfo->inputBuffSize = dec->current_frame_map.size;
#ifdef USE_TARGET_GPU
  cinfo->start_of_data = dec->current_frame_map.data;
#endif
...
gst_jpeg_dec_skip_input_data (j_decompress_ptr cinfo, glong num_bytes)
{
  ...
  if (num_bytes > 0 && cinfo->src->bytes_in_buffer >= (unsigned long)num_bytes) {
...
gst_jpeg_dec_init (GstJpegDec * dec)
{
  ...
  dec->bMeasure_ImageProcessTime = FALSE;
  dec->max_errors = JPEG_DEFAULT_MAX_ERRORS;
  dec->bEnable_rgba_output = FALSE;
  dec->bEnable_nv12_output = FALSE;
  dec->is_nvmm_format = FALSE;
}
...
gst_jpeg_dec_ensure_buffers (GstJpegDec * dec, guint maxrowbytes)
{
  ...
  if (G_LIKELY (dec->idr_width_allocated == (gint)maxrowbytes))
    return TRUE;
...
static gboolean
gst_jpeg_dec_negotiate (GstJpegDec * dec, gint width, gint height)
{
  ...
  GstCapsFeatures *ift;
  struct jpeg_decompress_struct cinfo = dec->cinfo;
  gint clrspc = cinfo.jpeg_color_space;
  ...
  switch (clrspc) {
    ...
    case JCS_YCbCr:
      if (cinfo.num_components == 3) {
        ...
        /* NV12 works only with I420 inputs */
        if (cinfo.comp_info[0].h_samp_factor == 2 &&
  ...
  templ_caps =
      gst_pad_get_pad_template_caps (GST_VIDEO_DECODER_SRC_PAD
      (GST_VIDEO_DECODER (dec)));
  intersection =
      gst_pad_peer_query_caps (GST_VIDEO_DECODER_SRC_PAD (GST_VIDEO_DECODER
          (dec)), templ_caps);
  ...
  gst_caps_append (comp_supported_caps,
      gst_video_info_to_caps (&outstate->info));

  gst_caps_append (comp_supported_caps,
      gst_video_info_to_caps (&outstate->info));

  ift = gst_caps_features_new ("memory:NVMM", NULL);
  gst_caps_set_features (comp_supported_caps, 0, ift);
  ...
  if ((dec->bEnable_rgba_output || dec->bEnable_nv12_output)
        && gst_caps_is_empty (intersection)) {
    gst_caps_unref (intersection);
    intersection = gst_caps_copy (comp_supported_caps);
  }
  ...
  if (gst_caps_is_empty (intersection)) {
    gst_caps_unref (intersection);
    GST_ERROR_OBJECT (GST_VIDEO_DECODER (dec), "Empty caps");
    return FALSE;
  }

  GstCaps *tmp1;
  guint n = 0, cnt = 0, n1 = 0;
  cnt = gst_caps_get_size (intersection);

  for (n = 0; n < cnt; n++) {
    GstCapsFeatures *feature;
    feature = gst_caps_get_features (intersection, n);

    if (gst_caps_features_contains (feature, "memory:NVMM")) {
      GST_DEBUG_OBJECT (dec, "Setting encoder to use Nvmm buffer");
#if defined (USE_TARGET_TEGRA) || defined (USE_TARGET_GPU)
      dec->cinfo.IsVendorbuf = TRUE;
#endif
      dec->is_nvmm_format = TRUE;
      n1 = n;
      if (dec->bEnable_nv12_output && !dec->bEnable_rgba_output) {
        if ((gst_caps_features_contains (feature, "NV12") &&
              format == GST_VIDEO_FORMAT_NV12))
          break;
      }
    }
  ...
  tmp1 = gst_caps_copy_nth (intersection, n1);
  outstate->caps = tmp1;
...
gst_jpeg_dec_handle_frame (GstVideoDecoder * bdec, GstVideoCodecFrame * frame)
{
...
#ifdef USE_TARGET_TEGRA
  dec->cinfo.bMeasure_ImageProcessTime = dec->bMeasure_ImageProcessTime;
  if (dec->bEnable_rgba_output && dec->cinfo.jpeg_color_space == JCS_YCbCr)
    dec->cinfo.out_color_space = JCS_RGBA_8888;
  else if (dec->bEnable_nv12_output)
    dec->cinfo.num_components = 2;
#endif
  ...
  switch (dec->cinfo.jpeg_color_space) {
    ...
    case JCS_RGB:
      ...
      break;
    case JCS_YCbCr:
      if ((dec->cinfo.num_components != 3 && dec->cinfo.num_components != 2) ||
  ...
  if (!gst_jpeg_dec_negotiate (dec, width, height)) {
...
#if defined (USE_TARGET_TEGRA) || defined (USE_TARGET_GPU)
                    && (!dec->cinfo.IsVendorbuf)
#endif
                    )
            || dec->cinfo.comp_info[0].h_samp_factor != 2
            || dec->cinfo.comp_info[1].h_samp_factor != 1
            || dec->cinfo.comp_info[2].h_samp_factor != 1
            || dec->cinfo.comp_info[0].v_samp_factor != 2
            || dec->cinfo.comp_info[1].v_samp_factor != 1
            || dec->cinfo.comp_info[2].v_samp_factor != 1)
            && !dec->is_nvmm_format) {
  ...
  if (dec->cinfo.IsVendorbuf)
  {
      GstMapInfo map = GST_MAP_INFO_INIT;
      gst_buffer_map(frame->output_buffer, &map, GST_MAP_READ);
      gpointer surf =  malloc(map.size);
      memcpy(surf, map.data, map.size);
      gst_buffer_unmap(frame->output_buffer, &map);
      GQuark quark = g_quark_from_static_string("nvbufsurf");
      gst_mini_object_set_qdata(GST_MINI_OBJECT(frame->output_buffer),quark,surf,(GDestroyNotify)NvBufSurfaceDestroy);
  }
#endif
...
#if defined (USE_TARGET_TEGRA)
static gboolean
gst_jpeg_dec_open (GstVideoDecoder * bdec)
{
  return TRUE;
}
#endif
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_PERF_MEASUREMENT:
      dec->bMeasure_ImageProcessTime = g_value_get_boolean (value);
      break;
    case PROP_RGBA_OUTPUT:
      dec->bEnable_rgba_output = g_value_get_boolean (value);
      break;
    case PROP_NV12_OUTPUT:
      dec->bEnable_nv12_output = g_value_get_boolean (value);
      break;
#if defined (USE_TARGET_GPU)
    case PROP_GPU_DEVICE_ID:
      dec->gpu_id = g_value_get_uint (value);
      break;
#endif
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MAX_ERRORS:
      ...
      break;
    case PROP_PERF_MEASUREMENT:
      g_value_set_boolean (value, dec->bMeasure_ImageProcessTime);
      break;
    case PROP_RGBA_OUTPUT:
      g_value_set_boolean (value, dec->bEnable_rgba_output);
      break;
    case PROP_NV12_OUTPUT:
      g_value_set_boolean (value, dec->bEnable_nv12_output);
      break;
#if defined (USE_TARGET_GPU)
    case PROP_GPU_DEVICE_ID:
      g_value_set_uint (value, dec->gpu_id);
      break;
#endif
