...
ekb_uefi_pv_public_key = 3;
ekb_uefi_encryption_key = 4;
ekb_device_id_cert = 5;
...
def main():
    ...
    parser.add_argument('-in_sym_key', nargs=1, required=False, help="16-byte symmetric key file in hex format")
    parser.add_argument('-in_sym_key2', nargs=1, required=False, help="16-byte symmetric key file in hex format")
    parser.add_argument('-in_uefi_pubkey', nargs=1, required=False, help="a cert using for verify uefi signature")
    parser.add_argument('-in_uefi_enckey', nargs=1, required=False, help="a 16-byte symmetric key file in hex format")
    ...
    if chip == "t194":
        ...
        if args.oem_k2_key == None:
            raise Exception("oem_k2_key is not set, it is required for t234")
    ...
    if args.in_uefi_pubkey != None:
        in_content += pack_key(args.in_uefi_pubkey[0], ekb_uefi_pv_public_key)
    if args.in_uefi_enckey != None:
        in_content += pack_key(args.in_uefi_enckey[0], ekb_uefi_encryption_key)
