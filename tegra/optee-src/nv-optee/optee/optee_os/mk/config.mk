...
# NVMe RPMB file system support
CFG_NVME_RPMB_FS ?= n

# Device identifier used when CFG_NVME_RPMB_FS = y.
# The exact meaning of this value is platform-dependent. On Linux, the
# tee-supplicant process will open /dev/nvme<id>
CFG_NVME_RPMB_FS_DEV_ID ?= 0

# This config variable determines the number of entries read in from RPMB at
# once whenever a function traverses the NVMe RPMB FS. Increasing the default
# value has the following consequences:
...
# config files
CFG_NVME_RPMB_FS_RD_ENTRIES ?= 8

# Enables caching of FAT FS entries when set to a value greater than zero.
# When enabled, the cache stores the first 'CFG_NVME_RPMB_FS_CACHE_ENTRIES' FAT
# FS entries. The cache is populated when FAT FS entries are initially read in.
...
# CFG_NVME_RPMB_FS_RD_ENTRIES*sizeof(struct nvme_rpmb_fat_entry) bytes of heap memory
...
# CFG_NVME_RPMB_FS_CACHE_ENTRIES*sizeof(struct nvme_rpmb_fat_entry) bytes of heap memory
...
CFG_NVME_RPMB_FS_CACHE_ENTRIES ?= 0

# Print NVMe RPMB data frames sent to and received from the RPMB device
CFG_NVME_RPMB_FS_DEBUG_DATA ?= n

# Clear NVMe RPMB content at cold boot
CFG_NVME_RPMB_RESET_FAT ?= n

# Use a hard coded NVMe RPMB key instead of deriving it from the platform HUK
CFG_NVME_RPMB_TESTKEY ?= n

# RPMB file system support
CFG_RPMB_FS ?= n

# Enable roll-back protection of REE file system using RPMB.
# Roll-back protection only works if CFG_RPMB_FS = y or CFG_NVME_RPMB_FS = y.
CFG_REE_FS_INTEGRITY_RPMB ?= $(call cfg-one-enabled, CFG_RPMB_FS CFG_NVME_RPMB_FS)
$(eval $(call cfg-depends-one,CFG_REE_FS_INTEGRITY_RPMB,CFG_RPMB_FS CFG_NVME_RPMB_FS))
...
_CFG_WITH_SECURE_STORAGE := $(call cfg-one-enabled,CFG_REE_FS CFG_RPMB_FS CFG_NVME_RPMB_FS)
