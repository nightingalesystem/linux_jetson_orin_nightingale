...
#define MEM_READ_ACCESS		BIT(0)
#define MEM_WRITE_ACCESS	BIT(1)
#define MEM_EXECUTABLE_ACCESS	BIT(2)
...
static struct device_region_dt stmm_dt_device_regions[MAX_DEVICE_REGIONS];
static uint32_t stmm_dt_device_regions_len = 0;
...
static TEE_Result map_region(struct stmm_ctx *ctx, uint64_t pa,
			     size_t num_pgs, uint32_t prot, vaddr_t *va)
{
	struct mobj *mobj = NULL;
	TEE_Result res = TEE_SUCCESS;

	mobj = mobj_phys_alloc(pa, num_pgs * SMALL_PAGE_SIZE,
			TEE_MATTR_MEM_TYPE_DEV, CORE_MEM_DEVICE);
	if (mobj == NULL)
		return TEE_ERROR_OUT_OF_MEMORY;

	res = vm_map(&ctx->uctx, va, num_pgs * SMALL_PAGE_SIZE, prot,
			VM_FLAG_EPHEMERAL, mobj, 0);
	if (res)
		mobj_put(mobj);
...
static TEE_Result stmm_sp_map_device_regions(struct stmm_ctx *ctx,
		struct stmm_boot_info *boot_info)
{
	TEE_Result ret = TEE_SUCCESS;
	uint32_t i = 0;
	struct device_region *dr = NULL;
	struct device_region_dt *drd = NULL;
	vaddr_t va;

	boot_info->device_regions_len = 0;
	for (i = 0; i < stmm_dt_device_regions_len; i++) {
		dr = &boot_info->device_regions[i];
		drd = &stmm_dt_device_regions[i];

		memset(dr->name, 0, DEVICE_REGION_NAME_LEN);
		memcpy(dr->name, drd->name, strlen(drd->name));
		dr->size = drd->page_count * SMALL_PAGE_SIZE;

		va = 0;
		ret = map_region(ctx, drd->pa, drd->page_count, drd->prot, &va);
		if (ret != TEE_SUCCESS) {
			EMSG("Map device: %s, pa: 0x%lx, sz: 0x%x, prot: 0x%x failed: 0x%x",
				dr->name, drd->pa, drd->page_count, drd->prot, ret);
			break;
		}
		dr->va = va;

		DMSG("Mapped device: %s, pa: 0x%lx, "
		     "sz: 0x%x, prot: 0x%x to 0x%lx",
		     dr->name, drd->pa, drd->page_count, drd->prot, (uint64_t)va);
		boot_info->device_regions_len++;
	}
...
static TEE_Result load_stmm(struct stmm_ctx *spc)
{
	...
	*boot_info = (struct stmm_boot_info){
	...
	res = stmm_sp_map_device_regions(spc, boot_info);
	if (res != TEE_SUCCESS)
		return res;
				...
				uint64_t *value)
{
	...
	cuint = fdt_getprop(fdt, node, property, &len);
	if (!cuint || len != sizeof(uint64_t))
		return TEE_ERROR_ITEM_NOT_FOUND;

	*value = ((uint64_t)fdt32_to_cpu(cuint[0])) << 32 |
...
static TEE_Result stmm_sp_save_dt_device_regions(void *fdt, int off)
{
	int sp_node = 0;
	int count = 0;
	...
	fdt_for_each_subnode(sp_node, fdt, off) {
		...
		if (count > MAX_DEVICE_REGIONS) {
			EMSG("Too many device regions in DT.");
			return TEE_ERROR_OUT_OF_MEMORY;
		}

		if (sp_dt_get_u64(fdt, sp_node, "base-address", &pa)) {
			EMSG("No device region base-address defined, skip.");
			continue;
		}

		if (sp_dt_get_u32(fdt, sp_node, "pages-count", &page_count)) {
			EMSG("No device region pages-count defined, skip.");
			continue;
		}

		if (sp_dt_get_u32(fdt, sp_node, "attributes", &attributes)) {
			EMSG("No device region attributes defined, skip.");
			continue;
		}
		...
		if (attributes & MEM_WRITE_ACCESS)
			prot |= TEE_MATTR_UW;

		/* Device memory should not be executable */
		if (attributes & MEM_EXECUTABLE_ACCESS) {
			EMSG("Device region mapped with executable, skip.");
			continue;
		}

		/* Enable secure memory when NS memory bit is not set */
		if (!(attributes & MEM_NS_ACCESS)) {
			prot |= TEE_MATTR_SECURE;
		} else {
			EMSG("Device region mapped with NS access, skip.");
			continue;
		}

		prot |= TEE_MATTR_DEVICE;
		prot |= TEE_MATTR_MEM_TYPE_DEV << TEE_MATTR_MEM_TYPE_SHIFT;

		region_name = fdt_get_name(fdt, sp_node, &name_len);
		if (!region_name || name_len >= DEVICE_REGION_NAME_LEN) {
			EMSG("Invalid device region name, skip.");
			continue;
		}

		drd = &stmm_dt_device_regions[count];
		memset(drd->name, 0, DEVICE_REGION_NAME_LEN);
		memcpy(drd->name, region_name, name_len);
		drd->pa = pa;
		drd->page_count = page_count;
		drd->prot = prot;

		DMSG("stmm_sp: parsed a device region: %s, pa: 0x%lx, page_count: %u, "
		     "prot: 0x%x", drd->name, drd->pa, drd->page_count, drd->prot);
		count++;
	}

	stmm_dt_device_regions_len = count;
	return TEE_SUCCESS;
}

bool stmm_is_device_mem_inside(paddr_t pa, size_t len)
{
	uint32_t i = 0;
	bool ret = false;

	for (i = 0; i < stmm_dt_device_regions_len; i++) {
		struct device_region_dt *drd = &stmm_dt_device_regions[i];
		if (core_is_buffer_inside(pa, len, drd->pa,
				drd->page_count * SMALL_PAGE_SIZE)) {
			ret = true;
			break;
		}
	}
...
static TEE_Result stmm_sp_parse_dt(void)
{
	void *fdt = NULL;
	int sp_node = 0;
	int node = 0;
	...
	fdt_for_each_subnode(sp_node, fdt, node) {
		...
		if (cuint32) {
			...
			if (memcmp(&dt_uuid, &stmm_uuid, sizeof(stmm_uuid)))
				continue;

			return stmm_sp_save_dt_device_regions(fdt, sp_node);
		}
	}

	return TEE_ERROR_ITEM_NOT_FOUND;
}
service_init(stmm_sp_parse_dt);
