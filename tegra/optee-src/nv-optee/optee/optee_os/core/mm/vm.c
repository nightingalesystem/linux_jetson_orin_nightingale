		      ...
		      size_t align)
{
	...
	if (!(prot & TEE_MATTR_DEVICE) && (prot & ~TEE_MATTR_PROT_MASK))
		return TEE_ERROR_BAD_PARAMETERS;

	/* Check If the device memory parameters are correct.*/
	if (prot & TEE_MATTR_DEVICE) {
		/* Only StMM SPs can map device memory.*/
		if (!is_stmm_ctx(uctx->ts_ctx))
			return TEE_ERROR_BAD_PARAMETERS;

		if ((prot & ~TEE_MATTR_DEVICE_PROT_MASK))
			return TEE_ERROR_BAD_PARAMETERS;
	}
