...
void core_mmu_set_secure_memory(paddr_t base, size_t size)
{
	...
	IMSG("Physical secure memory base %#"PRIxPA" size %#zx", base, size);
...
#ifdef CFG_WITH_STMM_SP
	case CORE_MEM_DEVICE:
		/* CORE_MEM_DEVICE is used by StMM only */
		return stmm_is_device_mem_inside(pbuf, len);
#endif
	default:
		return false;
	}
